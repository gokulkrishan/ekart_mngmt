class Api::V1::UsersController < ApplicationController
    ...
    def logout
        ...
        if user
          ...
          render json: "Invalid email id", status: 404
        end 
    end
    ...
    def forget_password
        user = User.find_by(email: params[:email])
        if user
            user.update(token: SecureRandom.hex(13), expire_at: Time.now + 2.hours)
    ...
    def reset_password
        ...
        if user && user.expire_at < Time.now
            ...
            render json: "Your password has been reset", status: 200
        else
            render json: "Invalid or expired password reset token", status: 404
        end
    end
